...
description: Analyze a codebase and recommend agent automations (hooks, subagents, skills, MCP servers).
type: prompt
whenToUse: When the user asks for automation recommendations, wants to optimize their agent CLI setup, or improve agent workflows.
disableModelInvocation: false
tools: Read, Glob, Grep, Bash
---

# Kimi Automation Recommender

Analyze codebase patterns to recommend tailored Kimi CLI automations across all extensibility options.

**This skill is read-only.** It analyzes the codebase and outputs recommendations. It does NOT create or modify any files. Users implement the recommendations themselves or ask Kimi separately to help build them.
...
| **Skills** | Packaged expertise, workflows, and repeatable tasks (invoked by Kimi or user via `/skill-name`) |
...
# Check for existing Kimi CLI config
...
| Code style | **project-conventions** | Kimi-only |
...
```markdown
## Kimi CLI Automation Recommendations
...
**Invocation**: User-only / Both / Kimi-only
...
- `user-invocable: false` — Kimi-only (for background knowledge)
...
Recommend headless Kimi for automated pipelines:
